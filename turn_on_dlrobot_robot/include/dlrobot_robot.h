...
class turn_on_robot
{
	...
	private:
		...
		ros::Subscriber Cmd_Vel_Sub,Imu_sub; //Initialize the topic subscriber //初始化话题订阅者
		//The speed topic subscribes to the callback function
		//速度话题订阅回调函数
		void Cmd_Vel_Callback(const geometry_msgs::Twist &twist_aux);              

		ros::Publisher odom_publisher, imu_publisher, voltage_publisher,sonar_publisher; //Initialize the topic publisher //初始化话题发布者
        ...
        float Power_voltage,distance;       //Power supply voltage //电源电压
		geometry_msgs::TransformStamped transformStamped_;
		bool publish_tf_,sub_imu_;
