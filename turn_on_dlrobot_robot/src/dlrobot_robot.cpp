...
{
  ros::init(argc, argv, "dlrobot_robot"); //ROS initializes and sets the node name //ROS初始化 并设置节点名称 
...
void turn_on_robot::Imu_Callback(const sensor_msgs::Imu &Imu)
{
    Robot_Vel.Z = Imu.angular_velocity.z;	         
}
/**************************************
Date: January 28, 2021
...
void turn_on_robot::Publish_Odom()
{
    ...
    //把Z轴转角转换为四元数进行表达
 //   geometry_msgs::Quaternion odom_quat = tf2::createQuaternionMsgFromYaw(Robot_Pos.Z);
	tf2::Quaternion q;
  ...
  //  odom.pose.pose.orientation = odom_quat;/
   ...
   else if (use_ekf_location == true)
   {
	...
	if (publish_tf_){
            ...
            transformStamped_.header.frame_id = "odom_combined";
            transformStamped_.child_frame_id = robot_frame_id;
            transformStamped_.transform.translation.x = Robot_Pos.X;
            transformStamped_.transform.translation.y = Robot_Pos.Y;
...
void turn_on_robot::Publish_Sonar()
{
     sensor_msgs::Range Sonar_Data_Pub; //Instantiate IMU topic data //实例化IMU话题数据
     Sonar_Data_Pub.header.stamp = ros::Time::now(); 
     Sonar_Data_Pub.header.frame_id = robot_frame_id; //IMU corresponds to TF coordinates, which is required to use the robot_pose_ekf feature pack 
     Sonar_Data_Pub.range = distance;     //The power supply voltage is obtained //电源供电的电压获取
     sonar_publisher.publish(Sonar_Data_Pub); //Post the power supply voltage topic unit: V, volt //发布电源电压话题单位：V、伏特   
}
...
{ 
  ...
  */
  ...
  if (Receive_Data.Frame_Header == FRAME_HEADER ) //Judge the frame header //判断帧头
  {
    ...
    { 
      if (Receive_Data.rx[RECEIVE_DATA_SIZE-2] == Check_Sum(RECEIVE_DATA_SIZE-2,READ_DATA_CHECK)) //BCC check passes or two packets are interlaced //BCC校验通过或者两组数据包交错
      {
        Receive_Data.Flag_Stop=Receive_Data.rx[1]; //set aside //预留位
        Robot_Vel.X = Odom_Trans(Receive_Data.rx[2],Receive_Data.rx[3])*linear_scale_; //Get the speed of the moving chassis in the X direction //获取运动底盘X方向速度
        Robot_Vel.Y = Odom_Trans(Receive_Data.rx[4],Receive_Data.rx[5])*linear_scale_; //Get the speed of the moving chassis in the Y direction, The Y speed is only valid in the omnidirectional mobile robot chassis
                                                                         //获取运动底盘Y方向速度，Y速度仅在全向移动机器人底盘有效
		if(sub_imu_ == false)
        Robot_Vel.Z = Odom_Trans(Receive_Data.rx[6],Receive_Data.rx[7])*angle_scale_; //Get the speed of the moving chassis in the Z direction //获取运动底盘Z方向速度   
  ...
  */  
  ...
  if(count == RECEIVE_DATA_SIZE) //Verify the length of the packet //验证数据包的长度
  {
    ...
    if(Receive_Data.Frame_Tail == FRAME_TAIL) //Verify the frame tail of the packet //验证数据包的帧尾
    {
     ...
     //   Robot_Vel.Y = Odom_Trans(Receive_Data.rx[4],Receive_Data.rx[5])*linear_scale_; //Get the speed of the moving chassis in the Y direction, The Y speed is only valid in the omnidirectional mobile robot chassis
                                                                          //获取运动底盘Y方向速度，Y速度仅在全向移动机器人底盘有效
         distance = Odom_Trans(Receive_Data.rx[4],Receive_Data.rx[5]);
        //MPU6050 stands for IMU only and does not refer to a specific model. It can be either MPU6050 or MPU9250
        //Mpu6050仅代表IMU，不指代特定型号，既可以是MPU6050也可以是MPU9250
        Mpu6050_Data.accele_x_data = IMU_Trans(Receive_Data.rx[8],Receive_Data.rx[9]);   //Get the X-axis acceleration of the IMU     //获取IMU的X轴加速度  
        Mpu6050_Data.accele_y_data = IMU_Trans(Receive_Data.rx[10],Receive_Data.rx[11]); //Get the Y-axis acceleration of the IMU     //获取IMU的Y轴加速度
     ...
     //   Mpu6050.linear_acceleration.y = Mpu6050_Data.accele_y_data / ACCEl_RATIO;
     //   Mpu6050.linear_acceleration.z = Mpu6050_Data.accele_z_data / ACCEl_RATIO;
	    Mpu6050.linear_acceleration.x = 0;
        Mpu6050.linear_acceleration.y =0;
		...
		if(sub_imu_ == true){
			if(Receive_Data.rx[2] == 0 && Receive_Data.rx[3] == 0 && Receive_Data.rx[6] == 0 && Receive_Data.rx[7] == 0  ){

				 motionless++;
				 angle_bias_total +=  Mpu6050.angular_velocity.z;
			 }
			 else{ 
				   motionless = 0; 
				   angle_bias_total = 0;
			 ...
			 if(Receive_Data.rx[2] == 0 && Receive_Data.rx[3] == 0 && Receive_Data.rx[6] == 0 && Receive_Data.rx[7] == 0  ){
				 ...
				 Robot_Vel.Z = 0;
				   ...
				   motionless = 0; 
				   angle_bias_total = 0;
				   Robot_Vel.Z = Mpu6050.angular_velocity.z - angle_bias;

				} 
			
				
		}
		if(motionless > 40)  angle_bias =  angle_bias_total / motionless;
	    Mpu6050.angular_velocity.z = Mpu6050.angular_velocity.z - angle_bias;
        transition_16 = 0;
        transition_16 |=  Receive_Data.rx[20]<<8;
        transition_16 |=  Receive_Data.rx[21];  
        Power_voltage = transition_16/1000+(transition_16 % 1000)*0.001; //Unit conversion millivolt(mv)->volt(v) //单位转换毫伏(mv)->伏(v)
...
void turn_on_robot::Control()
{
  ...
  while(ros::ok())
  {
    ...
    {
      _Now = ros::Time::now();     
	  if(_Last_Time.toSec()==0) _Last_Time=_Now; //Perform this operation when entering for the first time to avoid excessive integration time       //ʗ´νøȫʱ½øА´˲ٗ÷£¬±܃⻽·֊±¼乽´󠠠   
...
turn_on_robot::turn_on_robot():Sampling_Time(0),Power_voltage(0)
{
  ...
  private_nh.param<float>("linear_scale", linear_scale_,1.0);
  private_nh.param<float>("angle_scale", angle_scale_,1.0);
  private_nh.param<bool>("use_ekf_location", use_ekf_location,true);
  private_nh.param<std::string>("gyro_frame_id",    gyro_frame_id,    "imu_link"); //IMU topics correspond to TF coordinates //IMU话题对应TF坐标
  private_nh.param<bool>("publish_tf", publish_tf_, true);
  private_nh.param<bool>("sub_imu", sub_imu_, true);
  ...
  sonar_publisher   = n.advertise<sensor_msgs::Range>("sonar", 20); //Create an distance topic publisher //创建IMU话题发布者
  ...
  Imu_sub = n.subscribe("/imu/data_raw", 100, &turn_on_robot::Imu_Callback, this);//自适应蒙特卡洛定位需要的数据   
  ...
  {
    ROS_ERROR_STREAM("dlrobot_robot can not open serial port,Please check the serial port cable! "); //If opening the serial port fails, an error message is printed //如果开启串口失败，打印错误信息
  }
  if(Stm32_Serial.isOpen())
  {
    ROS_INFO_STREAM("dlrobot_robot serial port opened"); //Serial port opened successfully //串口开启成功提示
  }
}
/**************************************
Date: January 28, 2021
