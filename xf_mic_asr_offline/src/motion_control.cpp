...
void goal_reach_Callback(const move_base_msgs::MoveBaseActionResult& msg)
{
	...
	{
		system("aplay -D plughw:CARD=Device,DEV=0 ~/dlrobot_robot/src/xf_mic_asr_offline/feedback_voice/reach_goal.wav");
		goal_control=0;
	}
	else if(reach == "Goal reached." && overmap_flag==1)//建图完成播报
	{
		system("aplay -D plughw:CARD=Device,DEV=0 ~/dlrobot_robot/src/xf_mic_asr_offline/feedback_voice/overmap.wav");
		overmap_flag=0;
	}
		...
		if(cmd_vel_flag || turn_fin_flag)    //底盘运动控制部分
		{
			...
			if(distance_judgment() && dis_angleX_judgment())    //判断障碍物的距离和方向
			{
				...
				if(temp_count > radar_count)    //连续计数5️次停止运动防止碰撞，避免雷达有噪点
				{
					...
					turn_fin_flag = 0;
					temp_count = 0;
					system("aplay -D plughw:CARD=Device,DEV=0 ~/dlrobot_robot/src/xf_mic_asr_offline/feedback_voice/find.wav");
					}
				}
			}
